...
class EmotionVideo(Base):
    ...
    poste_id = Column(Integer)
    candidat_id = Column(Integer)
...
def detect_emotions():
    ...
    try:
        poste_id = int(request.form['poste'])
    except (ValueError, KeyError):
        print("Erreur: le paramètre 'poste' n'est pas un entier valide")
        return jsonify({"error": "Invalid 'poste' parameter"}), 400
    
    try:
        candidat_id = int(request.form['candidat'])
    except (ValueError, KeyError) as e:
        error_message = f"Erreur: le paramètre 'candidat' n'est pas un entier valide - {e}"
        print(error_message)
